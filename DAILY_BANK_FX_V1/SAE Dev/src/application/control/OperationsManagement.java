...
public class OperationsManagement {
	...
	public OperationsManagement(Stage _parentStage, DailyBankState _dbstate, Client client, CompteCourant compte) {
		...
		try {
			...
			System.out.println("test OperationsManagement.OperationsManagement");
			e.printStackTrace();
		}
	}

	public void doOperationsManagementDialog() {
		System.out.println("test  OperationsManagement.doOperationsManagementDialog av");
		this.omc.displayDialog(); //erreur..
		System.out.println("test  OperationsManagement.doOperationsManagementDialog ap");
	}
		...
		if (op != null) {
			try {
				...
				if(this.dbs.isChefDAgence() && (this.compteConcerne.solde - op.montant < this.compteConcerne.debitAutorise)) {
					...
					Alert a = new Alert(AlertType.CONFIRMATION, "", oui, non);
					a.setTitle("Confirmation");
					a.setResizable(true);
					a.setContentText("Vous êtes sur le point d'effectuer un débit exceptionnel (dépassement du seuil de découvert autorisé sur ce compte). Êtes-vous certain(e)?");
					a.showAndWait();
					if(a.getResult() == oui) {
						ao.insertDebitEx(this.compteConcerne.idNumCompte, op.montant, op.idTypeOp);
					} else {
